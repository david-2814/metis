...
import pytest
from metis_cli.runtime import _configure_file_logging
...
def _reset_logging_state(monkeypatch):
    ...
    import metis_cli.runtime as runtime

    monkeypatch.setattr(runtime, "_FILE_LOGGING_CONFIGURED", False)
    metis_logger = logging.getLogger("metis_core")
...
def test_configure_file_logging_idempotent(tmp_path, monkeypatch):
    ...
    _configure_file_logging()
    handlers_after_first = list(logging.getLogger("metis_core").handlers)
    _configure_file_logging()
    handlers_after_second = list(logging.getLogger("metis_core").handlers)
...
def test_configure_file_logging_creates_parent_dirs(tmp_path, monkeypatch):
    ...
    _configure_file_logging()
    logging.getLogger("metis_core").warning("creates parents")
    for h in logging.getLogger("metis_core").handlers:
        h.flush()
...
def test_adapter_warning_lands_in_log_file(tmp_path, monkeypatch):
    ...
    adapter_logger = logging.getLogger("metis_core.adapters.openrouter")
    ...
    assert "metis_core.adapters.openrouter" in contents
