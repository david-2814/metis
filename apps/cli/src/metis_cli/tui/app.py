...
class MetisApp(App):
    ...
    def on_mount(self) -> None:
        ...
        _write_wrapped(
            log,
            "Commands: /model <alias|id>, /model -, /cost, /models, /copy, /help. Ctrl-D to quit.",
        )
    ...
    def _handle_slash(self, text: str) -> None:
        ...
        if cmd == "/model":
            ...
            try:
                ...
                if resolved != arg:
                    _write_wrapped(log, f"Active model: {resolved}   (matched from {arg!r})")
                else:
        ...
        if cmd == "/models":
            ...
            self.runtime.manager.buffer_slash_output(self.session.id, "\n".join(rendered))
            return
        if cmd == "/share":
    ...
    def _handle_copy_command(self, arg: str, log: Log) -> None:
        ...
        if n > len(assistant_messages):
            _write_wrapped(log, f"only {len(assistant_messages)} assistant message(s) so far")
            return
