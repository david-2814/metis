...
import openai
import pytest
from metis_core.adapters.errors import AuthError, CancelledError, RateLimitError
from metis_core.adapters.openrouter import (
    OpenRouterAdapter,
    _parse_capabilities,
    _parse_pricing,
    _wire_model_name,
)
from metis_core.adapters.protocol import CanonicalRequest, StopReason
...
from metis_core.canonical.messages import Message, MessageMetadata, Role
...
def _composed_message(body: dict) -> str:
    from metis_core.adapters.openrouter import _provider_message
...
def test_provider_message_extracts_raw_json_string():
    ...
    raw_inner = (
        '{"error":{"message":"Function calling not supported","type":"invalid_request_error"}}'
    )
    body = {
        "error": {
            "message": "Provider returned error",
    ...
    assert msg == ("Provider returned error (Fireworks: Function calling not supported)")
...
def test_provider_message_raw_only_no_provider_name():
    ...
    assert _composed_message(body) == ("Provider returned error (upstream: Tokens limit exceeded)")
...
def test_provider_message_flat_top_level_message_in_raw():
    ...
    assert _composed_message(body) == ("Provider returned error (ProviderX: Service unavailable)")
...
def test_provider_message_nested_error_msg_field():
    ...
    assert _composed_message(body) == ("Provider returned error (SomeProvider: internal failure)")
...
def test_provider_message_priority_message_wins_over_msg():
    ...
    body = {
        "error": {
            "message": "Provider returned error",
            "metadata": {
                "raw": ('{"error":{"message":"canonical message","msg":"asian-style msg"}}'),
...
def test_provider_message_request_id_in_nested_error():
    ...
    body = {
        "error": {
            "message": "Provider returned error",
            "metadata": {
                "raw": ('{"error":{"message":"validation failed","request_id":"nested-id"}}'),
                "provider_name": "P",
            },
        }
    }
