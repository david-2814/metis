...
async def test_get_session_routing_policy_version_surfaces_when_loaded(client, workspace, runtime):
    ...
    from metis.core.routing.policy_loader import parse_policy_text
...
async def test_confirmation_resolved_happy_path(client, workspace, runtime):
    ...
    from metis.core.canonical.tools import SideEffects
    from metis.core.tools.confirmation import ConfirmationRequest
    from metis.server.app import build_app
    ...
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as c:
        sid = (await c.post("/sessions", json={"workspace_path": str(workspace)})).json()["id"]
        r = await c.post(
