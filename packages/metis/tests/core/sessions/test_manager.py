...
def _build_manager(
    ...
    *,
    ...
    from metis.core.tools.builtins.file_ops import ListDirTool, ReadFileTool
...
async def test_bare_alias_with_no_body_raises_override_error(bus, event_log, workspace):
    ...
    from metis.core.sessions import OverrideError
...
async def test_get_session_with_sqlite_store_returns_fresh_record(bus, workspace, tmp_path):
    ...
    from metis.core.sessions import SqliteSessionStore
...
async def test_set_active_model_auto_resolves_unambiguous_suffix(bus, workspace):
    ...
    from metis.core.sessions.manager import AmbiguousModelError  # noqa: F401
...
async def test_set_active_model_ambiguous_raises_with_candidates(bus, workspace):
    ...
    from metis.core.sessions.manager import AmbiguousModelError
...
def _whitespace_normalize(text: str) -> str:
    ...
    from metis.core.sessions.manager import _normalize_shared_text
...
def test_pad_stable_prefix_byte_stable_across_calls():
    ...
    from metis.core.sessions.manager import _pad_stable_prefix_for_cache
...
async def test_session_manager_pads_short_default_prompt(bus, event_log, workspace):
    ...
    """
    from metis.core.sessions.manager import MIN_CACHEABLE_PREFIX_TOKENS
...
async def test_session_manager_does_not_pad_already_long_prompt(bus, event_log, workspace):
    ...
    from metis.core.sessions.manager import _OPERATING_CONTEXT_PADDING
...
async def test_turn_completed_signals_extra_feeds_llm_judge_build_user_message(
    ...
    from metis.core.eval.llm_judge import _build_user_message
