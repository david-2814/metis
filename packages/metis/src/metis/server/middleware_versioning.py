...
The agent server has no provider-shape surface (those live on the gateway);
every route here is Metis-owned and therefore versioned. The middleware
reads ``Metis-API-Version`` from the request (default ``CURRENT_VERSION``),
echoes it on the response, stamps the comma-separated
``Metis-API-Versions-Supported`` discovery header, and:
