...
| 2026-05-17 | OSS license + repo strategy: Apache-2.0 single OSS repo (`metis`); private `metis-pro` for paid-tier code | Closes [`pricing.md §9.5`](specs/pricing.md) (was "out of scope"). Two-repo "thin Pro repo" pattern: the OSS substrate is standalone-usable (gateway + canonical IR + adapters + routing + pattern store + bounded memory + tools + skills + heuristic evaluator + per-key analytics + agent CLI/TUI/serve). `metis-pro` holds operationally-sensitive surfaces (billing, signup, accounts store, hosted dashboard UI, curated LLM-judge rubric library, enterprise SAML/OIDC/SCIM glue). OSS defines extension Protocols (`BillingBackend`, `SignupBackend`, `AnalyticsExtension`, `JudgeRubricProvider`) with noop defaults; Pro overlays implement them at boot. Rationale: (a) buyer trust signal is load-bearing pre-revenue — Apache-2.0 is the cleanest OSI-approved permissive signal; (b) the four-leg moat (§4) is operational/compounding, not source-level — comparable projects (LiteLLM, Supabase, PostHog) survive permissive licensing because the moat lives in roadmap velocity + accumulated buyer data + brand, not source secrecy; (c) the "thin Pro repo" pattern keeps cross-cutting friction low because most pricing.md §7.2 Pro features are access-restricted (route handlers, rollups) rather than code-restricted; (d) operational discipline — billing / payment / accounts logic genuinely belongs with operational infrastructure, not with the agent product; (e) reversible — single-repo merge or BUSL relicense remain on the table if a fork-and-SaaS threat materializes (2028+ problem at current scale). Concrete migration plan: [`docs/operations/repo-split-plan.md`](operations/repo-split-plan.md). No code moves yet — the migration is scheduled, not executed. Pricing.md §9.5 retired; §12 decision log updated. |
