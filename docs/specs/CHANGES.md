...
- **Status:** verified for the additive scope. **Implementation deferred.** Phase 4 implementation is gated on §A3-rev3 outcome; if §A3-rev3 inverts slot 4 under v1, v2 stays specs-only and this entry becomes documentation of a designed-but-unbuilt path. Update this entry's status to "implemented" only when `metis_core.patterns.embedders` lands and §16.10 test 5 (headline cluster-tightening test) passes.
