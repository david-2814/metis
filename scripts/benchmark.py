...
_ALLOWED_TOP = {"name", "description", "suite_version", "turns", "expect"}
...
async def run_workload(
    ...
    *,
    ...
    actual_model: str,
    temperature: float | None,
) -> tuple[int, int, list[dict], Decimal, int, int]:
    """Drive one workload end-to-end. Returns (started_us, ended_us,
    per_turn_metrics, total_cost_usd, total_llm_calls, total_tool_calls).
    ...
    ended_us = _now_us()
    return started_us, ended_us, per_turn_metrics, total_cost, total_llm, total_tool
...
def _format_table(rows: list[WorkloadResult]) -> str:
    ...
    for r in rows:
        lines.append(
            ...
            f"{r.savings_usd:<9.6f}{r.savings_pct * 100:>6.1f}%"
        )
    return "\n".join(lines)
...
async def amain() -> int:
    ...
    if not args.skip_execute:
        for workload in workloads:
            try:
                started_us, ended_us, per_turn, cost, llm, tool = await run_workload(
                    workload,
                    db_path=db_path,
                    actual_model=actual_resolved,
                    temperature=args.temperature,
