    ...
    *,
    ...
    if not isinstance(data, dict):
        raise PolicyValidationError(
            ["routing policy root must be a mapping"], source=source_path
        )
...
def parse_policy(
    ...
    *,
    ...
    rules = _parse_rules(
        raw.get("rules") or [], scope="global", registry=registry, errors=errors
    )
    workspaces = _parse_workspaces(
        raw.get("workspaces") or {}, registry=registry, errors=errors
    )
...
def _parse_workspaces(
    ...
    *,
    ...
    for path, cfg in raw.items():
        ...
        rules = [
            Rule(name=r.name, when=r.when, use=r.use, scope="workspace") for r in rules
        ]
        workspaces.append(
            WorkspaceScope(
...
def _parse_predicate(
    ...
    if key == "skills_matching_message_includes":
        return _parse_string_list(
            value, prefix, errors, factory=SkillsMatchingMessageIncludes
        )
...
def _check_model_ref(
    field: str, model: Any, registry: ModelRegistry, errors: list[str]
) -> None:
