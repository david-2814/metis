...
def load_skills(
    *,
    ...
    workspace_skills = (
        _load_dir(workspace_dir, source="workspace") if workspace_dir else {}
    )
    ...
    for name, skill in workspace_skills.items():
        if name in merged:
            logger.info(
                "skill %r in workspace overrides global definition", name
            )
        merged[name] = skill
...
def _load_skill(skill_dir: Path, *, source: SkillSource) -> Skill:
    ...
    if compatibility is not None:
        if not isinstance(compatibility, str):
            ...
            errors.append(
                f"compatibility: must be <=500 chars (got {len(compatibility)})"
            )
    ...
    if allowed_tools_raw is not None:
        if not isinstance(allowed_tools_raw, str):
            ...
            allowed_tools_parsed = tuple(
                t for t in allowed_tools_raw.split() if t
            )
