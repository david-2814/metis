...
- **Multi-user / team-level rollups** — gateway v1 is single-tenant in shape (one workspace per key); analytics has no `team_id` or `tenant_id` concept yet. Per-key cost roll-ups are wired (`/analytics/cost?group_by=gateway_key`, `/analytics/by_key`); multi-workspace per key and team aggregations are Phase 3 follow-on per `gateway.md §11`.
