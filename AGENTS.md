...
- **Pattern store v2 — hybrid embedding fingerprint (Wave 10)** — Opt-in alternative to the v1 structural fingerprint per [`pattern-store.md §16`](docs/specs/pattern-store.md). Module [`packages/metis-core/src/metis_core/patterns/embeddings.py`](packages/metis-core/src/metis_core/patterns/embeddings.py) defines a `@runtime_checkable` `EmbeddingProvider` Protocol and three concrete providers (`openai:text-embedding-3-small` 1536-dim, `cohere:embed-multilingual-v3.0` 1024-dim, `local:sentence-transformers:all-MiniLM-L6-v2` 384-dim) plus a `DeterministicEmbeddingProvider` for tests and a `resolve_embedding_provider(provider_id)` registry. `PatternStore` gains an additive `embedding_cache(text_sha256, provider_id, embedding_blob, embedding_dim, …)` table — keyed `(provider_id, SHA-256(user_message_text))`, vector blobs packed via `array.array('f', …).tobytes()` (no NumPy dep), bounded by `embedding_cache_max_rows=10_000` + `embedding_cache_max_age_days=180` with age-first → LRU → use-count tie-break eviction. Blended similarity: `α × cosine + (1 − α) × weighted_jaccard` with default `α = 0.6`; mixed-version K-NN (v1 row vs v2 query) falls back to v1 weighted-Jaccard. Opt in via `PatternConfig.fingerprint_version="v2"` + `embedding_provider="<provider_id>"` in `routing.yaml`; v1 default is unchanged. Routing slot 4 in v2 mode does a sync cache-only lookup before computing the query fingerprint — cache hit → blended K-NN; cache miss → v1 jaccard. Recording path is async via `attach_embedding_for_recording()`. Schema_version bumps `"1" → "2"` in place; no row migration. 52 new tests under `packages/metis-core/tests/patterns/` covering Protocol contract, cosine/blend math, cache TTL + LRU eviction, mixed-version K-NN, recording-path cache-first embed, routing slot-4 v2 path. Deviations from the original spec recorded in pattern-store.md §16.13 (rename `embedding_blend_alpha → embedding_alpha`, no `embedding_strategy` knob, sync `recommend()` preserved, no NumPy hard dep).
...
- **Pattern store v2 cluster-tightening A/B** — pattern-store.md §16.10 test 5 ("intra-cluster similarity ≥ 0.10 higher AND inter-cluster ≥ 0.05 lower under v2 than v1 on the 60-turn fixture") is the headline gate for v2 paying for itself; not yet run on the curated fixture spanning the 6 benchmark workloads + 4 agent-loop traces.
