...
from metis.routing.predicates import _in_window, evaluate
...
def test_has_tool_calls_in_history():
    assert (
        evaluate(HasToolCallsInHistory(expected=True), _ctx(has_tool_calls_in_history=True))
        is True
    )
    assert (
...
def test_workspace_path_matches():
    ...
    assert (
        evaluate(WorkspacePathMatches(pattern=re.compile(r"^/Users/")), ctx) is True
    )
    assert (
        evaluate(WorkspacePathMatches(pattern=re.compile(r"^/Code/")), ctx) is False
    )
...
def test_skills_predicate_returns_false_in_v1():
    ...
    assert (
        evaluate(SkillsMatchingMessageIncludes(skill_names=("anything",)), ctx) is False
    )
...
def test_time_of_day_uses_now_override():
    ...
    assert (
        evaluate(TimeOfDayBetween(start_minutes=14 * 60, end_minutes=15 * 60), ctx)
        is True
    )
    assert (
        evaluate(TimeOfDayBetween(start_minutes=15 * 60, end_minutes=16 * 60), ctx)
        is False
    )
...
def test_time_of_day_window_wraps_midnight():
    ...
    assert (
        evaluate(TimeOfDayBetween(start_minutes=22 * 60, end_minutes=6 * 60), ctx)
        is True
    )
    # 12:00 noon is NOT in [22:00, 06:00].
    ctx2 = _ctx(now_override=datetime(2026, 5, 11, 12, 0))
    assert (
        evaluate(TimeOfDayBetween(start_minutes=22 * 60, end_minutes=6 * 60), ctx2)
        is False
    )
