...
def test_configure_file_logging_idempotent(tmp_path, monkeypatch):
    ...
    _configure_file_logging()
    handlers_after_first = list(logging.getLogger("metis").handlers)
    _configure_file_logging()
    handlers_after_second = list(logging.getLogger("metis").handlers)
...
def test_configure_file_logging_creates_parent_dirs(tmp_path, monkeypatch):
    ...
    _configure_file_logging()
    logging.getLogger("metis").warning("creates parents")
    for h in logging.getLogger("metis").handlers:
        h.flush()

    assert log_path.exists()


def test_configure_file_logging_bad_path_warns_not_raises(
...
def test_adapter_warning_lands_in_log_file(tmp_path, monkeypatch):
    ...
    adapter_logger = logging.getLogger("metis.adapters.openrouter")
    ...
    assert "metis.adapters.openrouter" in contents
