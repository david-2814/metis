...
from metis.cli.models_display import format_models_lines
from metis.pricing.table import ModelPricing, PriceTable
...
def test_unpriced_model_shows_dash():
    ...
    lines = format_models_lines(
        ["openrouter:obscure/model"], registry=reg, pricing=pricing
    )
...
def test_priced_model_shows_in_and_out_rates():
    ...
    lines = format_models_lines(
        ["anthropic:claude-sonnet-4-6"], registry=reg, pricing=pricing
    )
