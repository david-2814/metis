...
from metis.adapters.streaming import MessageComplete, MessageStart, TextDelta
...
from metis.skills import register_skill_tools
from metis.skills.store import load_skills
...
async def test_discovery_injected_into_system_prompt(bus, workspace, skill_dir):
    ...
    manager = _build_manager(
        bus,
        adapter,
        skill_store_factory=lambda ws: load_skills(
            global_dir=skill_dir, workspace_dir=None
        ),
    )
    session = manager.create_session(workspace_path=str(workspace))
...
async def test_empty_skill_store_does_not_add_section(bus, workspace, tmp_path):
    ...
    manager = _build_manager(
        ...
        adapter,
        skill_store_factory=lambda ws: load_skills(
            global_dir=empty_dir, workspace_dir=None
        ),
    )
    session = manager.create_session(workspace_path=str(workspace))
    await manager.submit_turn(session.id, "hello")
